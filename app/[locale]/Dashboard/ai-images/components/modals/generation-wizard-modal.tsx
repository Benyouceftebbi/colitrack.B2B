...
// Re-define internal types to avoid direct import issues if app/page types are complex
...
const STEPS = {
  ...
  reel: [
    { id: "sourceImage", title: "Source Image (Optional)" },
    { id: "detailsSettings", title: "Details & Settings" },
    { id: "review", title: "Review & Generate" },
  ],
}
...
export function GenerationWizardModal({
  ...
  const [inspirationImageFile, setInspirationImageFile] = useState<File | null>(null) // For image mode
  ...
  const steps = STEPS[generationType]
    ...
    if (currentStepIndex < steps.length - 1) {
      ...
      const data =
        generationType === "image"
          ? { prompt, settings: imageSettings, productImage: productImageFile, inspirationImage: inspirationImageFile }
          : { prompt, settings: reelSettings, productImage: productImageFile } // productImage is source for reel
      onSubmit(data)
    }
  }
  ...
  const renderStepContent = () => {
    ...
    switch (stepId) {
      ...
      case "detailsSettings": // Image Generation - Step 2 OR Reel Generation - Step 2
        if (generationType === "image") {
          return (
            <div className="space-y-6">
              <div>
      ...
      case "sourceImage": // Reel Generation - Step 1
        return (
          <div className="space-y-6">
            <FileUploadArea
              file={productImageFile} // Using productImageFile for reel source
              onFileChange={setProductImageFile}
              isDragActive={isDragActiveProduct}
              setIsDragActive={setIsDragActiveProduct}
              inputRef={productFileInputRef}
              title="Product Picture (Optional)"
      ...
      case "review":
        return (
          <div className="space-y-4">
            ...
            <div className="p-4 bg-muted rounded-md space-y-2 text-sm border">
              <p>
                <strong>Type:</strong> <Badge variant="outline">{generationType === "image" ? "Image" : "Reel"}</Badge>
              </p>
              {productImageFile && (
                <p>
                  <strong>Product Picture:</strong> {productImageFile.name} (
                  {(productImageFile.size / 1024 / 1024).toFixed(2)} MB)
                </p>
              )}
  ...
  const isNextDisabled = () => {
    ...
    if (generationType === "image") {
      ...
      // Reel
      // No mandatory fields in reel step 1 (sourceImage is optional)
      if (stepId === "detailsSettings" && !prompt.trim()) return true
    }
    return false
  }
    ...
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-xl md:max-w-2xl lg:max-w-3xl max-h-[90vh] flex flex-col p-0">
            ...
            <div className="flex items-center justify-center space-x-2 sm:space-x-4 pt-4">
              {steps.map((step, index) => (
                <div key={step.id} className="flex flex-col items-center">
                  ...
                  >
                    {index < currentStepIndex ? (
                      <Check className="w-5 h-5 animate-in fade-in zoom-in-100 duration-300 ease-out" />
                    ) : (
                      index + 1
                    )}
                  </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 min-h-[350px]">{renderStepContent()}</div>
