...
interface NEWExcelImportModalProps {
  isOpen: boolean
  onClose: () => void
}

export function NEWExcelImportModal({ isOpen, onClose }: NEWExcelImportModalProps) {
  ...
  const parseExcel = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setErrorMessage("")
        setSelectedColumn("") // Reset column selection for manual selection
      } catch (error) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      let functionName = ""
      if (shopData.deliveryCompany === "E-COM Delivery") {
        functionName = "fetchEcomTrackingData"
      } else if (shopData.deliveryCompany === "ZR express") {
        functionName = "fetchZRTrackingData"
      }

      const importTracking = httpsCallable(functions, functionName)
            ...
            {columns.length > 0 && (
              <div className="space-y-4 mt-4">
                <div className="grid gap-2">
                  <Label htmlFor="column-select">{t("select-tracking-column") || "Select tracking ID column"}</Label>
                ...
                {selectedColumn && (
                  <div className="bg-muted/50 p-3 rounded-md mt-4">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">{t("selected-column") || "Selected column:"}</span>
