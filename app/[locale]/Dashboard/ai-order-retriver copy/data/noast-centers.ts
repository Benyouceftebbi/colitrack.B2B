...
interface NoastCenter {
  ...
  export function getNoastCenterById(centerId: string): NoastCenter | undefined {
    for (const commune of Object.values(noastCenters)) {
      const center = commune.centers.find((center) => center.code === centerId || center.code === centerId)
  ...
  // Add a function to get the commune ID directly from the noast-centers data structure
  export function getNoestCommuneId(wilayaName: string): number | undefined {
    if (!wilayaName) return undefined
  
    const normalizedWilayaName = normalizeString(wilayaName).toLowerCase().trim()
  
    // Try to find an exact match first
    for (const communeId in noastCenters) {
      const commune = noastCenters[communeId]
      if (normalizeString(commune.wilaya_name_ascii).toLowerCase() === normalizedWilayaName) {
        return commune.id // Return the numeric ID directly
      }
    }
  ...
  // Helper function to get commune ID for a specific wilaya and commune
  export function getNoastCommuneIdWithCommune(wilayaName: string, communeName?: string): number | undefined {
    if (!wilayaName) return undefined
