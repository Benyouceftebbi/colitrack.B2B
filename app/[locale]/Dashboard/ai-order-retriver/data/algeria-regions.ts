// This file contains the regions of Algeria with the updated structure

export interface AlgeriaRegion {
    id: number
  ...
  export const algeriaRegions: AlgeriaRegion[] =[
    {
        "id": 22,
  ...
  export function getAllWilayas(): { code: string; name_ascii: string; name: string }[] {
    ...
    algeriaRegions.forEach((region) => {
      if (!wilayasMap.has(region.wilaya_code)) {
        wilayasMap.set(region.wilaya_code, {
   ...
   * @param wilayaCode The code of the wilaya
   * @returns Array of communes in the specified wilaya
   */
  export function getCommunesByWilayaCode(wilayaCode: string): AlgeriaRegion[] {
    return algeriaRegions.filter((region) => region.wilaya_code === wilayaCode)
  }
  
  /**
   * Get all communes for a specific wilaya by name
  ...
  export function getCommunesByWilayaName(wilayaName: string): AlgeriaRegion[] {
    ...
    return algeriaRegions.filter(
      (region) =>
        normalizeString(region.wilaya_name_ascii).includes(normalizedName) ||
        normalizedName.includes(normalizeString(region.wilaya_name_ascii)) ||
        normalizeString(region.wilaya_name).includes(normalizedName) ||
        normalizedName.includes(normalizeString(region.wilaya_name)),
    )
  }
  
  /**
   * Find a commune by its name within a specific wilaya
   * @param wilayaCode The code of the wilaya
   * @param communeName The name of the commune (in ASCII/French or Arabic)
  ...
  export function findCommuneByName(wilayaCode: string, communeName: string): AlgeriaRegion | undefined {
    ...
    const exactMatch = algeriaRegions.find(
      (region) =>
        region.wilaya_code === wilayaCode &&
        (normalizeString(region.commune_name_ascii) === normalizedName ||
          normalizeString(region.commune_name) === normalizedName),
    )
    ...
    // If no exact match, try partial match (contains)
    return algeriaRegions.find(
      (region) =>
        region.wilaya_code === wilayaCode &&
        (normalizeString(region.commune_name_ascii).includes(normalizedName) ||
          normalizedName.includes(normalizeString(region.commune_name_ascii)) ||
          normalizeString(region.commune_name).includes(normalizedName) ||
          normalizedName.includes(normalizeString(region.commune_name))),
    )
  }
  
  /**
   * Find a commune by its name across all wilayas
   * @param communeName The name of the commune (in ASCII/French or Arabic)
   * @returns The commune if found, undefined otherwise
   */
