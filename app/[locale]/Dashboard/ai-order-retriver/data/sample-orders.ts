// Sample data for demonstration purposes with the new structure
export const orders = [
    {
      id: "ORD-001",
      source: "messenger",
      recipientId: "29125428493769811",
      senderId: "414297458433296",
      status: "retrived",
      timestamp: "2025-04-05T00:28:59+0100",
      storePage: "SabyAnge Fashion",
      conversation: [
        {
          message: "Salam ch7al hadak TAE lkerch ch7al ydir",
          sender: "client",
          sentAt: "2025-04-04T00:32:29+0000",
          type: "text",
        },
        {
          message: "سلام اختي مرحبا 🌹",
          sender: "shop",
          sentAt: "2025-04-04T00:33:19+0000",
          type: "text",
        },
        {
          message: "سعر مشد البطن 4 امتار دار 150 افف",
          sender: "shop",
          sentAt: "2025-04-04T00:33:23+0000",
          type: "text",
        },
        {
          attachment:
            "https://scontent-dfw5-2.xx.fbcdn.net/v/t1.15752-9/471891025_589506093687423_7454640593244125495_n.jpg?_nc_cat=106&ccb=1-7&_nc_sid=fc17b8&_nc_eui2=AeHJSMZVWgcGVFflVl0EdLGghX5rpueKjFmFfmum54qMWd9VI9_UPQhGGWWgk2CNrhHfFP3uhAvmEOK8CTUJg4aY&_nc_ohc=mYFjTsEIMyEQ7kNvwECSW6f&_nc_oc=AdnsnciLSII4X2OW4hoLfmWa1naKakAu4Cv-d4eVmb4ZX9_MtM5ARRXHo1uSNPfMEBK2RrBQ_h88dQk5uvOKCxbQ&_nc_zt=23&_nc_ht=scontent-dfw5-2.xx&edm=AB0BvfgEAAAA&oh=03_Q7cD2AFDJZQYSvzdJS6swzCgOK2plnN1fo9mRXD0tGY__eHguQ&oe=6817DF48",
          message: "image",
          sender: "shop",
          sentAt: "2025-04-04T00:33:30+0000",
          type: "image",
        },
        {
          message: "W la livraison",
          sender: "client",
          sentAt: "2025-04-04T00:34:51+0000",
          type: "text",
        },
        {
          message: "Mafihch",
          sender: "client",
          sentAt: "2025-04-04T00:35:00+0000",
          type: "text",
        },
        {
          message: "كاين اختي واش من ولاية أنتي",
          sender: "shop",
          sentAt: "2025-04-04T00:37:02+0000",
          type: "text",
        },
        {
          message: "35",
          sender: "client",
          sentAt: "2025-04-04T00:37:15+0000",
          type: "text",
        },
        {
          message: "Boumerdes",
          sender: "client",
          sentAt: "2025-04-04T00:37:19+0000",
          type: "text",
        },
        {
          message: "توصل للمكتب 35 الف و للمنزل 50 الف",
          sender: "shop",
          sentAt: "2025-04-04T00:37:41+0000",
          type: "text",
        },
        {
          message: "Maelich twrili nchofo ml dakhel🙂",
          sender: "client",
          sentAt: "2025-04-04T00:38:44+0000",
          type: "text",
        },
        {
          message: "نعم اختي بالطبع",
          sender: "shop",
          sentAt: "2025-04-04T00:38:52+0000",
          type: "text",
        },
        {
          attachment:
            "https://scontent-dfw5-1.xx.fbcdn.net/v/t1.15752-9/476481540_9468523943183548_6393760926639551087_n.jpg?_nc_cat=105&ccb=1-7&_nc_sid=fc17b8&_nc_eui2=AeE6A4JyaMgH1N5o_nOdiF_0R8-i0ZCGjwpHz6LRkIaPCiqofcnhgIuNeSKyELBSeN-jibQcxvBpHUzHA44BL9y1&_nc_ohc=mD4qT6P7ETcQ7kNvwFkL_qM&_nc_oc=Adm2MP2HAQObDhLp-gF81mg4w1H1qN-VNjOvWIkkfIY-1q-16MwPa9a7zhQJslXkaOh05H4BSsR8hKkO6IXl8I14&_nc_zt=23&_nc_ht=scontent-dfw5-1.xx&edm=AB0BvfgEAAAA&oh=03_Q7cD2AHuF9bXmFPcSOzwEkFAtsdH_m_eQTodMgzVYN1VXZ3zcQ&oe=6817CAAB",
          message: "image",
          sender: "shop",
          sentAt: "2025-04-04T00:38:58+0000",
          type: "image",
        },
        {
          message: "قماشو خشين ماشي رقيق ليفسد",
          sender: "shop",
          sentAt: "2025-04-04T00:39:11+0000",
          type: "text",
        },
        {
          message: "Khlas ghedwa nchlh nrj3lk blkhber w nbeatlk les infos",
          sender: "client",
          sentAt: "2025-04-04T00:40:25+0000",
          type: "text",
        },
        {
          message: "ان شاء الله اختي مرحبا بيك 🌹",
          sender: "shop",
          sentAt: "2025-04-04T00:40:44+0000",
          type: "text",
        },
        {
          message: "Salam",
          sender: "client",
          sentAt: "2025-04-04T22:31:39+0000",
          type: "text",
        },
        {
          message: "Tched mlih yak khoya",
          sender: "client",
          sentAt: "2025-04-04T22:31:46+0000",
          type: "text",
        },
        {
          message: "نعم اختي تشد مليح دخل الكرش و الخصر",
          sender: "shop",
          sentAt: "2025-04-04T22:39:03+0000",
          type: "text",
        },
        {
          message: "Wchmn bureau tb3toholi?",
          sender: "client",
          sentAt: "2025-04-04T22:54:56+0000",
          type: "text",
        },
        {
          message: "اسم مكتب دياشدي اكسبرس",
          sender: "shop",
          sentAt: "2025-04-04T22:56:53+0000",
          type: "text",
        },
        {
          message: "راهو جاي في حي 350 مسكن",
          sender: "shop",
          sentAt: "2025-04-04T22:57:01+0000",
          type: "text",
        },
        {
          message: "Ok nbeatlk asmi la9bi w nmr de tlph haka",
          sender: "client",
          sentAt: "2025-04-04T23:02:57+0000",
          type: "text",
        },
        {
          message: "نعم اختي",
          sender: "shop",
          sentAt: "2025-04-04T23:03:07+0000",
          type: "text",
        },
        {
          message: "Amina aitsilarbi 0542838967",
          sender: "client",
          sentAt: "2025-04-04T23:03:35+0000",
          type: "text",
        },
        {
          message:
            "تم تاكيد الطلبية بارك الله فيكم شكرا لثقتكم نعدك بمنتج في القمة ستصلك في أقرب وقت ممكن وان شاء الله نكونو عند حسن ضنكم وثقتكم بنا ❤️🌹",
          sender: "shop",
          sentAt: "2025-04-04T23:04:30+0000",
          type: "text",
        },
        {
          message: "الأحد تكون عندك بحول الله",
          sender: "shop",
          sentAt: "2025-04-04T23:04:33+0000",
          type: "text",
        },
        {
          message: "✅",
          sender: "shop",
          sentAt: "2025-04-04T23:28:21+0000",
          type: "text",
        },
      ],
      orderData: {
        additional_information: {
          confidence: 0.85,
          description: "Delivery is expected on Sunday.",
          value: "Delivery is expected on Sunday.",
        },
        address: {
          confidence: 0.8,
          value: "اسم مكتب دياشدي اكسبرس راهو جاي في حي 350 مسكن",
        },
        articles: [
          {
            name: {
              confidence: 0.95,
              value: "مشد البطن",
            },
            colors: [],
            price_details: [
              {
                price: {
                  confidence: 0.95,
                  value: 15000,
                },
                quantity: {
                  confidence: 0.95,
                  value: 1,
                },
              },
            ],
            quantity: {
              confidence: 0.95,
              value: 1,
            },
            sizes: [
              {
                confidence: 0.85,
                value: "4 امتار",
              },
            ],
            total_article_price: {
              confidence: 0.95,
              value: 15000,
            },
          },
        ],
        client_name: {
          confidence: 0.99,
          value: "Amina Aitsilarbi",
        },
        commune: {
          commune_probability: [
            {
              commune_name: "حي 350 مسكن",
              probability: 0.7,
            },
          ],
          name_ar: {
            confidence: 0.7,
            value: "حي 350 مسكن",
          },
          name_fr: {
            confidence: 0.7,
            value: "Hai 350 Maskan",
          },
        },
        delivery_cost: {
          confidence: 0.9,
          value: 3500,
        },
        delivery_date: {
          confidence: 0.85,
          value: "الأحد",
        },
        delivery_type: {
          confidence: 0.9,
          value: "stopdesk",
        },
        message_time: {
          confidence: 0.95,
          value: "2025-04-04T23:04:30+0000",
        },
        phone_number: {
          confidence: 0.99,
          value: "0542838967",
        },
        total_price: {
          confidence: 0.9,
          value: 18500,
        },
        wilaya: {
          name_ar: {
            confidence: 0.95,
            value: "بومرداس",
          },
          name_fr: {
            confidence: 0.95,
            value: "Boumerdes",
          },
        },
      },
    },
    // Adding more sample orders with the same structure
    {
      id: "ORD-002",
      source: "messenger",
      recipientId: "29125428493769812",
      senderId: "414297458433297",
      status: "pending",
      timestamp: "2025-04-04T22:15:30+0100",
      storePage: "SabyAnge Fashion",
      conversation: [
        {
          message: "Salam, est-ce que vous avez des AirPods Pro?",
          sender: "client",
          sentAt: "2025-04-04T21:30:15+0000",
          type: "text",
        },
        {
          message: "Oui, nous les avons en stock. Le prix est de 35000 DA.",
          sender: "shop",
          sentAt: "2025-04-04T21:32:45+0000",
          type: "text",
        },
        {
          message: "Je voudrais en commander une paire. Livraison à Oran possible?",
          sender: "client",
          sentAt: "2025-04-04T21:35:10+0000",
          type: "text",
        },
        {
          message: "Oui, la livraison à Oran est disponible. 500 DA pour le bureau et 800 DA à domicile.",
          sender: "shop",
          sentAt: "2025-04-04T21:37:22+0000",
          type: "text",
        },
        {
          message: "Je préfère la livraison au bureau. Mon nom est Fatima Zahra, téléphone 0555789012.",
          sender: "client",
          sentAt: "2025-04-04T21:40:05+0000",
          type: "text",
        },
      ],
      orderData: {
        additional_information: {
          confidence: 0.8,
          description: "Client prefers office delivery",
          value: "Client prefers office delivery",
        },
        address: {
          confidence: 0.85,
          value: "Bureau de poste central, Oran",
        },
        articles: [
          {
            name: {
              confidence: 0.98,
              value: "AirPods Pro",
            },
            colors: [],
            price_details: [
              {
                price: {
                  confidence: 0.98,
                  value: 35000,
                },
                quantity: {
                  confidence: 0.98,
                  value: 1,
                },
              },
            ],
            quantity: {
              confidence: 0.98,
              value: 1,
            },
            sizes: [],
            total_article_price: {
              confidence: 0.98,
              value: 35000,
            },
          },
        ],
        client_name: {
          confidence: 0.99,
          value: "Fatima Zahra",
        },
        commune: {
          commune_probability: [
            {
              commune_name: "Oran",
              probability: 0.9,
            },
          ],
          name_ar: {
            confidence: 0.9,
            value: "وهران",
          },
          name_fr: {
            confidence: 0.9,
            value: "Oran",
          },
        },
        delivery_cost: {
          confidence: 0.95,
          value: 500,
        },
        delivery_type: {
          confidence: 0.95,
          value: "stopdesk",
        },
        message_time: {
          confidence: 0.95,
          value: "2025-04-04T21:40:05+0000",
        },
        phone_number: {
          confidence: 0.99,
          value: "0555789012",
        },
        total_price: {
          confidence: 0.95,
          value: 35500,
        },
        wilaya: {
          name_ar: {
            confidence: 0.98,
            value: "وهران",
          },
          name_fr: {
            confidence: 0.98,
            value: "Oran",
          },
        },
      },
    },
    {
      id: "ORD-003",
      source: "instagram",
      recipientId: "29125428493769813",
      senderId: "414297458433298",
      status: "delivered",
      timestamp: "2025-04-03T15:45:22+0100",
      storePage: "SabyAnge Fashion",
      conversation: [
        {
          message: "Bonjour, avez-vous le Samsung Galaxy S22 en stock?",
          sender: "client",
          sentAt: "2025-04-03T14:20:15+0000",
          type: "text",
        },
        {
          message: "Oui, nous l'avons en noir, blanc et vert. 256GB à 120000 DA.",
          sender: "shop",
          sentAt: "2025-04-03T14:22:30+0000",
          type: "text",
        },
        {
          attachment: "https://placeholder.svg?height=400&width=400",
          message: "image",
          sender: "shop",
          sentAt: "2025-04-03T14:23:15+0000",
          type: "image",
        },
        {
          message: "Je veux le noir. Livraison à Constantine possible?",
          sender: "client",
          sentAt: "2025-04-03T14:25:45+0000",
          type: "text",
        },
        {
          message: "Oui, livraison à domicile à Constantine pour 1000 DA.",
          sender: "shop",
          sentAt: "2025-04-03T14:27:10+0000",
          type: "text",
        },
        {
          message:
            "Parfait. Mon nom est Karim Boudiaf, téléphone 0555456789, adresse 78 Rue Larbi Ben M'hidi, Appartement 3B, 2ème étage.",
          sender: "client",
          sentAt: "2025-04-03T14:30:22+0000",
          type: "text",
        },
      ],
      orderData: {
        additional_information: {
          confidence: 0.9,
          description: "Apartment 3B, 2nd floor",
          value: "Apartment 3B, 2nd floor",
        },
        address: {
          confidence: 0.95,
          value: "78 Rue Larbi Ben M'hidi",
        },
        articles: [
          {
            name: {
              confidence: 0.98,
              value: "Samsung Galaxy S22",
            },
            colors: [
              {
                confidence: 0.95,
                value: "noir",
              },
            ],
            price_details: [
              {
                price: {
                  confidence: 0.98,
                  value: 120000,
                },
                quantity: {
                  confidence: 0.98,
                  value: 1,
                },
              },
            ],
            quantity: {
              confidence: 0.98,
              value: 1,
            },
            sizes: [
              {
                confidence: 0.9,
                value: "256GB",
              },
            ],
            total_article_price: {
              confidence: 0.98,
              value: 120000,
            },
          },
        ],
        client_name: {
          confidence: 0.99,
          value: "Karim Boudiaf",
        },
        commune: {
          commune_probability: [
            {
              commune_name: "Constantine",
              probability: 0.95,
            },
          ],
          name_ar: {
            confidence: 0.95,
            value: "قسنطينة",
          },
          name_fr: {
            confidence: 0.95,
            value: "Constantine",
          },
        },
        delivery_cost: {
          confidence: 0.95,
          value: 1000,
        },
        delivery_type: {
          confidence: 0.95,
          value: "home",
        },
        message_time: {
          confidence: 0.95,
          value: "2025-04-03T14:30:22+0000",
        },
        phone_number: {
          confidence: 0.99,
          value: "0555456789",
        },
        total_price: {
          confidence: 0.95,
          value: 121000,
        },
        wilaya: {
          name_ar: {
            confidence: 0.98,
            value: "قسنطينة",
          },
          name_fr: {
            confidence: 0.98,
            value: "Constantine",
          },
        },
      },
    },
  ]
  
  // Define types for the order data structure
  export interface OrderConversation {
    message: string
    sender: "client" | "shop"
    sentAt: string
    type: "text" | "image"
    attachment?: string
  }
  
  export interface ConfidenceValue<T> {
    confidence: number
    value: T
  }
  
  export interface Article {
    name: ConfidenceValue<string>
    colors: Array<ConfidenceValue<string>>
    price_details: Array<{
      price: ConfidenceValue<number>
      quantity: ConfidenceValue<number>
    }>
    quantity: ConfidenceValue<number>
    sizes: Array<ConfidenceValue<string>>
    total_article_price: ConfidenceValue<number>
  }
  
  export interface Commune {
    commune_probability: Array<{
      commune_name: string
      probability: number
    }>
    name_ar: ConfidenceValue<string>
    name_fr: ConfidenceValue<string>
  }
  
  export interface OrderData {
    additional_information?: {
      confidence: number
      description: string
      value: string
    }
    address: ConfidenceValue<string>
    articles: Article[]
    client_name: ConfidenceValue<string>
    commune: Commune
    delivery_cost: ConfidenceValue<number>
    delivery_date?: ConfidenceValue<string>
    delivery_type: ConfidenceValue<string>
    message_time: ConfidenceValue<string>
    phone_number: ConfidenceValue<string>
    total_price: ConfidenceValue<number>
    wilaya: {
      name_ar: ConfidenceValue<string>
      name_fr: ConfidenceValue<string>
    }
  }
  
  export interface Order {
    id: string
    source: "messenger" | "instagram"
    recipientId: string
    senderId: string
    status: "retrived" | "pending" | "delivered"
    timestamp: string
    storePage: string
    conversation: OrderConversation[]
    orderData: OrderData
  }
  
  