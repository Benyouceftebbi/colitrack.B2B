// Define interfaces for the ZR/ECOM centers data structure
export interface ZrEcomCenter {
  ...
  export interface ZrEcomCommune {
    id: number
    ...
    wilaya_name: string
    centers: ZrEcomCenter[]
  }
  
  // This is a partial representation of the data
  // In a real implementation, this would be the complete dataset
  const zrEcomCenters: ZrEcomCommune[] = [
    {
      "id": 1,
    ...
    },
    {
      ...
      "centers": [
        {
          "code": "OUM EL BOUGHI - AIN EL BEIDA ",
        ...
        {
          "code": "OUM EL BOUGHI - VILLE LIVREUR",
    ...
    },
    {
      ...
      "centers": [
        {
          "code": "BENI ABBES CITE EL MOUSTAKBEL",
      ...
      "centers": [
        {
          ...
          "key": "GUELMA",
          "phones": {
            "0": "07 91 61 67 69 /07 70 96 22 21",
            "1": "",
            "2": "",
            "3": ""
          },
    ...
    },
    {
      ...
      "centers": [
        ...
        {
          "code": "CONSTANTINE - NOUVELLE VILLE ",
  ...
  /**
   * Get ZR/ECOM centers for a specific commune
   * @param communeName The name of the commune
   * @returns Array of ZR/ECOM centers for the commune
   */
  export function getZrEcomCentersForCommune(communeName: string): ZrEcomCenter[] {
    const normalizedCommuneName = normalizeString(communeName)
  
    const commune = zrEcomCenters.find((c) => normalizeString(c.commune_name_ascii) === normalizedCommuneName)
  
    return commune?.centers || []
  }
  
  /**
   * Get ZR/ECOM centers by wilaya name
   * @param wilayaName The name of the wilaya
   * @returns Array of ZR/ECOM centers for the wilaya
   */
  export function getZrEcomCentersByWilaya(wilayaName: string): ZrEcomCenter[] {
    const normalizedWilayaName = normalizeString(wilayaName)
  
    // Find all communes in the specified wilaya
    const communes = zrEcomCenters.filter((c) => normalizeString(c.wilaya_name_ascii) === normalizedWilayaName)
  
    // Flatten all centers from all communes in the wilaya
    return communes.reduce((allCenters, commune) => {
      return [...allCenters, ...commune.centers]
    }, [] as ZrEcomCenter[])
  }
  
  /**
   * Check if a commune has ZR/ECOM centers
   * @param communeName The name of the commune
   * @returns True if the commune has ZR/ECOM centers, false otherwise
   */
  export function hasZrEcomCenters(communeName: string): boolean {
    return getZrEcomCentersForCommune(communeName).length > 0
  }
  
  /**
   * Get a ZR/ECOM center by its key
   * @param key The key of the center
   * @returns The center if found, undefined otherwise
   */
  export function getZrEcomCenterByKey(key: string): ZrEcomCenter | undefined {
    for (const commune of zrEcomCenters) {
      const center = commune.centers.find((c) => c.key === key)
      if (center) {
        return center
      }
    }
  ...
  /**
   * Get a commune ID by wilaya name
   * @param wilayaName The name of the wilaya
   * @returns The commune ID if found, undefined otherwise
   */
  export function getZrEcomCommuneIdByWilaya(wilayaName: string): number | undefined {
    const normalizedWilayaName = normalizeString(wilayaName)
  
    const commune = zrEcomCenters.find((c) => normalizeString(c.wilaya_name_ascii) === normalizedWilayaName)
  
    return commune?.id
  }
  
  /**
   * Get a commune ID for a specific wilaya and commune
   * @param wilayaName The name of the wilaya
   * @param communeName The name of the commune
   * @returns The commune ID if found, undefined otherwise
   */
  export function getZrEcomCommuneId(wilayaName: string, communeName: string): number | undefined {
    const normalizedWilayaName = normalizeString(wilayaName)
    const normalizedCommuneName = normalizeString(communeName)
  
    const commune = zrEcomCenters.find(
      (c) =>
        normalizeString(c.wilaya_name_ascii) === normalizedWilayaName &&
        normalizeString(c.commune_name_ascii) === normalizedCommuneName,
    )
